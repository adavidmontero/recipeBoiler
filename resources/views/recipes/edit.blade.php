<x-app-layout>
    ...
    <div class="row">
        <div class="col-sm-4">
            <h5 class="font-weight-light">Edición de Receta</h5>
            <p class="text-muted">
                Modifica los datos para la actualización de esta receta.
            </p>
        </div>
                ...
                <div class="w-md-75">
                    <div class="mb-3">
                        ...
                        <div class="input-group date" id="datetimepicker1" data-target-input="nearest">
                            <input type="text" class="form-control datetimepicker-input" id="published-at" 
                                name="published_at" data-target="#datetimepicker1" value="{{ $recipe->published_at ? $recipe->published_at->format('m/d/Y H:i A') : '' }}" />
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" 
                            id="title" name="title" value="{{ old('title', $recipe->title) }}">
                        @error('title')
                            <span class="text-danger">
                                {{ $message }}
                            </span>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tags[]" class="select2 form-control {{ $errors->has('tags') ? 'is-invalid' : '' }}" 
                            ...
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" {{ $recipe->tags->contains($tag->id) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control {{ $errors->has('excerpt') ? 'is-invalid' : '' }}" 
                            id="excerpt" name="excerpt" rows="3">{{ old('excerpt', $recipe->excerpt) }}</textarea>
                        @error('excerpt')
                            <span class="text-danger">
                                {{ $message }}
                            </span>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="description" name="description" class="form-control 
                            {{ $errors->has('description') ? 'is-invalid' : '' }}">{{ old('description', $recipe->description) }}</textarea>
                        @error('description')    
                            <span class="text-danger">
                                {{ $message }}
                            </span>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="ingredients" name="ingredients" class="form-control 
                            {{ $errors->has('ingredients') ? 'is-invalid' : '' }}">{{ old('ingredients', $recipe->ingredients) }}</textarea>
                        @error('ingredients')    
                            <span class="text-danger">
                                {{ $message }}
                            </span>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="preparation" name="preparation" class="form-control 
                            {{ $errors->has('preparation') ? 'is-invalid' : '' }}">{{ old('preparation', $recipe->preparation) }}</textarea>
                        @error('preparation')    
                            <span class="text-danger">
                                {{ $message }}
                            </span>
